...
function getHandle(mpath = DEF.MPATH, subProce = null, mdir = '') {
	return (procing[subProce?.pid || process.pid] || new ProceHdl(subProce)).setMpath(assocJS(mpath), mdir);
}
...
function ProceHdl(subProce = null) {
	this.mpath = {};
	const pid = (subProce ? this.proce = subProce : process).on('message', getEar(this.liser = {})).pid
	pid ? procing[pid] = this : void 0;
}
ProceHdl.prototype = {
	/**@param {string} mdir */
